...
def generate_markdown_report(result: BenchmarkRunResult) -> str:
    ...
    if result.cross_category_duplicates:
        lines.append(
            f"- **Cross-category Collision Errors Excluded:** {len(result.cross_category_duplicates)}"
        )
    else:
    ...
    lines.append("| --- | --- | --- |")
    lines.append(
        f"| **Overall Accuracy** | `{result.accuracy * 100:.2f}%` | Fraction of total images classified correctly |"
    )
    lines.append(
        f"| **Precision (AI Generated)** | `{result.precision * 100:.2f}%` | When Chai predicts AI Generated, how often is it right? |"
    )
    lines.append(
        f"| **Recall (AI Generated)** | `{result.recall * 100:.2f}%` | How many of the actual AI Generated images did Chai catch? |"
    )
    lines.append(
        f"| **F1 Score (AI Generated)** | `{result.f1:.4f}` | Harmonic mean of AI Generated precision & recall |"
    )
    lines.append(
        f"| **Macro F1 Score** | `{result.macro_f1:.4f}` | Unweighted average across Real and AI Generated |"
    )
    lines.append(
        f"| **Weighted F1 Score** | `{result.weighted_f1:.4f}` | Support-weighted average F1 across classes |"
    )
    lines.append("")
    lines.append(
        f"*Counts:* **TP** = `{result.tp}`, **TN** = `{result.tn}`, **FP** = `{result.fp}`, **FN** = `{result.fn}`"
    )
    lines.append("")
    ...
    lines.append("| --- | --- |")
    lines.append(
        f"| **Average Confidence on Correct Predictions** | `{ca.mean_confidence_correct * 100:.2f}%` |"
    )
    lines.append(
        f"| **Average Confidence on Incorrect Predictions** | `{ca.mean_confidence_incorrect * 100:.2f}%` |"
    )
    lines.append(
        f"| **High-Confidence Failures (Confidence >= 80%)** | `{ca.high_confidence_failures_count}` cases |"
    )
    lines.append(
        f"| **Low-Confidence Correct (Confidence <= 60%)** | `{ca.low_confidence_correct_count}` cases |"
    )
    lines.append("")
    ...
    lines.append(
        f"### 7.1 False Positives ({len(fps)} Real images predicted as AI Generated)"
    )
    if fps:
        lines.append(
        ...
        for item in fps[:10]:
            ...
            lines.append(
                f"| `{item['image_id']}` | `{item['predicted_class']}` | `{item['confidence'] * 100:.1f}%` | `{top_det}` | `{Path(item['file_path']).name}` |"
            )
        if len(fps) > 10:
            lines.append(
                f"*... and {len(fps) - 10} more false positives recorded in JSON.*"
            )
    else:
    ...
    lines.append(
        f"### 7.2 False Negatives ({len(fns)} AI Generated images predicted as Real)"
    )
    if fns:
        lines.append(
            "| Image ID | Predicted | Confidence | Top Detectors | Relative Path |"
        ...
        if len(fns) > 10:
            lines.append(
                f"*... and {len(fns) - 10} more false negatives recorded in JSON.*"
            )
    else:
    ...
    lines.append(
        f"## High-Confidence Failures ({len(high_conf)} cases with Confidence >= 80%)"
    )
    lines.append("")
    if high_conf:
        lines.append(
            "| Image ID | Ground Truth | Predicted | Confidence | Top Detectors | File |"
        )
        lines.append("| --- | --- | --- | --- | --- | --- |")
        for item in high_conf[:10]:
            top_det = ", ".join(item.get("most_influential_detectors", [])[:2])
            lines.append(
                f"| `{item['image_id']}` | `{item['ground_truth']}` | `{item['predicted_class']}` | `{item['confidence'] * 100:.1f}%` | `{top_det}` | `{Path(item['file_path']).name}` |"
            )
    else:
    ...
    lines.append(
        f"## Low-Confidence Correct ({len(low_conf)} cases with Confidence <= 60%)"
    )
    lines.append("")
    if low_conf:
        ...
        for item in low_conf[:10]:
            lines.append(
                f"| `{item['image_id']}` | `{item['ground_truth']}` | `{item['predicted_class']}` | `{item['confidence'] * 100:.1f}%` | `{Path(item['file_path']).name}` |"
            )
    else:
